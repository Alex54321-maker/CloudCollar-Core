...
export default function WarehouseDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      setIsOnline(false);
      setLogs(prev => [
        { id: Date.now(), time: new Date().toLocaleTimeString(), type: 'CONFLICT', text: '[⚠️ NETWORK ERROR] Потеряна связь с ядром FastAPI' },
        ...prev.slice(0, 10)
      ]);
    }
  };
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto bg-slate-950 text-slate-100 min-h-screen font-mono selection:bg-blue-500 selection:text-white">

      {/* Шапка Кибер-Терминала */}
      <div className="flex justify-between items-center mb-6 bg-slate-900/60 border border-slate-800 p-5 rounded-2xl backdrop-blur-md shadow-2xl">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1 uppercase tracking-widest">Матрица распределения грузов автономного склада</p>
        </div>

        <div className={`flex items-center gap-2 border px-4 py-2 rounded-full text-xs font-bold tracking-widest transition-all ${
          isOnline 
        ...
        <div className="lg:col-span-2 bg-slate-900/40 border border-slate-800/80 p-6 rounded-2xl backdrop-blur-md shadow-2xl flex flex-col justify-between">
          ...
          <div className="grid grid-cols-2 gap-4 flex-grow content-start">
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-4 z-10">
                    {cellData.sku ? (
                      <>
                        <div className="text-sm font-bold text-slate-200 tracking-wide font-sans truncate">{cellData.sku}</div>
                        <div className="text-md font-extrabold text-white mt-1">
                          {cellData.weight} <span className="text-xs text-slate-500 font-normal">кг</span>
                        </div>
                      </>
                    ) : (
                      <div className="text-xs italic text-slate-600 tracking-wider">ЛОКАЦИЯ СВОБОДНА</div>
                    )}
                  </div>

                  {isHeavyA2 && (
                    <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-r from-red-600 to-rose-600 text-[10px] text-center font-black text-black py-0.5 uppercase tracking-widest z-10 shadow-lg">
            ...
            {Object.keys(cells).length === 0 && (
              <div className="col-span-2 text-center py-20 text-slate-600 italic tracking-widest uppercase text-xs animate-pulse">
        ...
        <div className="bg-slate-900/40 border border-slate-800/80 p-6 rounded-2xl backdrop-blur-md shadow-2xl">
          ...
          <div className="space-y-3">
            {activeRobots.map((robot) => (
              <div key={robot.id} className="p-4 bg-slate-950/60 border border-slate-800/60 rounded-xl flex justify-between items-center group hover:border-slate-700 transition-colors">
                <div>
                ...
                <span className={`text-[10px] px-2.5 py-1 rounded-md font-bold tracking-widest border ${
                  robot.status === 'MOVING' 
                    ? 'bg-purple-500/10 border-purple-500/30 text-purple-400 animate-pulse shadow-[0_0_10px_rgba(168,85,247,0.1)]' 
